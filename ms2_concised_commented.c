...
// Print "Fatal error" to stderr and exit with status 1
void fatal() {
    write(2, "Fatal error\n", 12);
    exit(1);
}
...
int extract_message(char **buf, char **msg) {
    if (!*buf) return 0; // No buffer, no message
    int i = 0;
    while ((*buf)[i] && (*buf)[i] != '\n') i++; // Find newline
    if (!(*buf)[i]) return 0; // No newline, incomplete message
    *msg = *buf;              // Set message to start of buffer
    (*msg)[i + 1] = 0;        // Null-terminate after newline
    char *newbuf = calloc(1, strlen(*buf + i + 1) + 1); // Allocate new buffer
    if (!newbuf) fatal();
    strcpy(newbuf, *buf + i + 1); // Copy remaining data
    *buf = newbuf;            // Update buffer
    return 1;                 // Message extracted
}
